...
- 侧边栏仅显示“分类 + 文件名”的纯文本列表；不使用图标映射与复杂标识。
- 单击文件打开编辑器；右键菜单提供基础操作（打开、插入、新建、删除、重命名）。
- 插入内容保持原文件缩进与格式，不更改编码与行尾符。
- 搜索仅匹配“文件名/类别路径”，通过命令式输入框进行前端过滤；不做内容搜索与拼音首字母匹配。
- 刷新行为采用“操作内刷新 + 手动刷新命令”，不进行文件系统自动监听与防抖处理。

## 15. 性能与健壮性（极简版）

- 文件枚举与读取需异步并批量控制，避免阻塞 UI；仅按后缀轻量过滤。
- 元数据与文件分离：记录 `size/createdAt/updatedAt/lastUsedAt`；不计算 `hash`，不监听保存事件。
- 对未知文件类型与损坏元数据采取“安全默认值 + 自动重建”。
- 避免背景监听与复杂缓存策略；在必要操作后主动刷新即可。

## 16. 安全与权限（极简版）

- 存储路径访问失败需明确提示并引导修复；设置新路径后仅创建基本目录结构。
- 插件不擅自修改用户项目目录结构；数据集中存储于用户配置路径。
- 路径安全边界：仅对库域内文件执行删除与重命名；对外部路径直接阻止并提示。
